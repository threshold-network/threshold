...
Assuming [Geth was installed](../../staking-and-running-a-node/running-a-node/self-managed/tbtc-v2-node-setup/operator-account.md) for the root user with the command provided in the [Operator Account creation](../../staking-and-running-a-node/running-a-node/self-managed/tbtc-v2-node-setup/operator-account.md) step, the operator-key file should be located in the `~/operator-key` directory.
