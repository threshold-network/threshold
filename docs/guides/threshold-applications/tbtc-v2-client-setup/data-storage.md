...
Assuming [Geth was installed](operator-account.md) for the root user with standard settings, the keystore file should be located in the `~/keystore` directory.

```bash
cd ~/keystore
```

Contained within the keystore directory is the account key file (operator key file), its name will be similar to the following: \
